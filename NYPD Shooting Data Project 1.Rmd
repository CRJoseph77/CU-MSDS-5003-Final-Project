...
shooting_data <- shooting_data %>% mutate(PRECINCT = as.factor(PRECINCT),JURISDICTION_CODE = as.factor(JURISDICTION_CODE), 
...
shootings_by_boro$BORO <- factor(shootings_by_boro$BORO, levels = c("BROOKLYN","BRONX", "QUEENS","STATEN ISLAND", "MANHATTAN"))
...
```{r shootings by year}
shootings_by_year <- shootings_by_year_boro %>% group_by(year) %>%
...
shootings_by_year %>% ggplot(aes(x = year, y = shootings, fill = year))+
  geom_bar(stat = "Identity") +
...
shootings_by_month <- shooting_data %>% mutate(month = month(OCCUR_DATE, label = TRUE)) %>% 
